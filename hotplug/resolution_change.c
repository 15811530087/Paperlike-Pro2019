...
get_outputs (void)
{
	...
	if (output->automatic)
	{
		...
		break;
	    case RR_Disconnected:
		printf("b output %s, disconnected.\n", output_info->name);
		if ((!(output->changes & changes_mode)))
		{
		    set_name_xid (&output->mode, None);
...
int search_target_output(char *output_name)
{
	int o = 0;
	...
	for (o = 0; o < res->noutput; o++)
	{
		...
		if (output_info->nmode > 1 && output_info->nmode < 4) {
			memcpy(output_name, output_info->name, strlen(output_info->name));
			return 0;
		}
	}
...
int
resolution_change(char *resolution)
{
    ...
    ret = search_target_output(output_name);
    if (ret == 1) {
	printf("cannot find target output:%s.\n", output_name);
	goto out;
    }
    //printf("output name %s \n", output_name);
    config_output = find_output_by_name (output_name);
    if (!config_output) {
	config_output = add_output();
	if (config_output == NULL)
		goto out;
	set_name (&config_output->output, output_name, name_string|name_xid);
    }
    set_name (&config_output->mode, resolution, name_string|name_xid);
